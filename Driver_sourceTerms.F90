...
subroutine Driver_sourceTerms(blockCount, blockList, dt, pass) 
    ...
    use Grid_data, ONLY: gr_smalle, gr_meshMe, gr_globalMe
...
#include "Flash_mpi.h"
 ...
 !merger
    ...
    !!merger if want particle to appear after relaxation

        !if (dr_simTime .lt. tinitial + sim_tRelax + 5.e1) then
        !  print *, 'dr_simTime [Driver_sourceTerms]',dr_simTime
        !  !print *, 'ptvecs',ptvecs
        ...
        !          do i = 1, localnpf
        !              if (idnint(particles_global(TAG_PART_PROP,i)) .ne. sim_fixedPartTag) then
        !                  print *, 'sim_fixedParticle', sim_fixedParticle
        !                  particles_global(POSX_PART_PROP:POSZ_PART_PROP,i) = ptvec(1:3)
        !              endif
        !          enddo
        !      else
        !          stvec = -ptvec
        ...
        !      print *,'[Driver_sourceTerms] particles_global',particles_global(POSX_PART_PROP:POSZ_PART_PROP,i)
        !      print *, 'sim_startDistance', ptvec(1), sim_startDistance
        !  endif
        !endif
