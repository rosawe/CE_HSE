...
#include "Particles.h"
    ...
    !sam: trim envelope
    !sim_lastRow is the index of the outermost radius that is being placed on the grid
    call RuntimeParameters_get('sim_lastRow',sim_lastRow)
    ...
    if (gr_globalMe .eq. MASTER_PE) then
        ...
        allocate(sim_table(sim_lastRow, sim_tableCols))
        allocate(sim_table1(sim_tableRows, sim_tableCols))
        call read_table(sim_profFile, sim_table1, sim_tableRows, sim_tableCols)
        !sam: trim envelope (next 2 lines)
        sim_table=sim_table1(:sim_lastRow,:)
        sim_tableRows = sim_lastRow
        !write(*,*) 'r_prof', sim_table(:,R_PROF)
        !write(*,*) 'rho_prof', sim_table(:,RHO_PROF)
        !write(*,*) 'temp_prof', sim_table(:,TEMP_PROF)
        write(*,*) 'Loaded stellar profile.'
    endif
    ...
    if (sim_startBeta .gt. 0.d0) then
        a = sim_periDist/(1.d0 - sim_orbEcc)
         ...
         sim_periTime = abs(ecc_anom - sim_orbEcc*dsin(ecc_anom))*period/2.d0/PI! + sim_tRelax !sam: comment out for merger
     endif
    ...
    if (gr_globalMe .eq. MASTER_PE) then
        ...
        if (dr_simTime .eq. sim_tInitial) then
            ...
            !sam: merger: comment out bc these are no longer calculated
